...
export default function Hero() {
  ...
  // The five trade chips reuse the first five shared service names, lowercased
  // for the hero's understated look.
  const serviceNames = t('serviceNames', { returnObjects: true })
  const serviceAreas = [0, 1, 2, 3, 11].map((i) => serviceNames[i])
    ...
    <section className={styles.hero} id="top">
      ...
      <div className={styles.inner}>
        ...
        >
          ...
          <motion.p variants={item}>{t('hero.intro')}</motion.p>
          ...
          <motion.div className={styles.searchBox} variants={item}>
            ...
            <div className={styles.examples} aria-label={t('hero.areasAria')}>
              {serviceAreas.map((area) => (
                <a key={area} href="#services">{area.toLowerCase()}</a>
              ))}
            </div>
          </motion.div>
        </motion.div>
